...
contract ViserionPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address constant public wbnbAddress = address(0xDeadDeAddeAddEAddeadDEaDDEAdDeaDDeAD0000);
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            if (remainingRewards > 0) {
                ...
                accRewardPerShare = accRewardPerShare.add(totalRewards.mul(1e24).div(lpSupply));
            }
        }
    ...
    function updatePool() public {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(totalRewards.mul(1e24).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
