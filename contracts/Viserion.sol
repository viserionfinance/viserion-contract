...
contract ViserionFinance is Ownable, ERC20 {
    using SafeMath for uint256;
    ...
    uint256 public maxSupply = 100e6 * 10**18;
    uint256 public capFarm = maxSupply.div(100).mul(75);

    INetswapRouter public netswapRouter;
    address public netswapPair;
    ...
    function setRouter(
        ...
        netswapPair = _pair;
        netswapRouter = INetswapRouter(_address);
        metis = _metis;
    }
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForBosses) public onlyOwner {
        require(_tokenForBosses < 500000 * 10**18);
    ...
    function _transfer(
        ...
        uint256 transferFeeRate = recipient == netswapPair
            ? sellFeeRate
            : (sender == netswapPair ? buyFeeRate : 0);
    ...
    function sweepTokenForBosses() public onlyBossCaller {
        ...
        if (contractTokenBalance >= tokenForBosses) {
            swapTokensForMetis(tokenForBosses);
        }
    }
    ...
    function swapTokensForMetis(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> metis
